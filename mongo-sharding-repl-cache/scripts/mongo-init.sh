...
docker exec -it configSrv mongosh --port 27017 --quiet
<<EOF
rs.initiate(
  {
    _id : "config_server",
...
docker exec -it shard1 mongosh --port 27018 --quiet
<<EOF
rs.initiate(
    {
      _id : "shard1",
...
docker exec -it shard2 mongosh --port 27019 --quiet
<<EOF
rs.initiate(
    {
      _id : "shard2",
...
docker exec -it mongos_router mongosh --port 27020 --quiet
<<EOF
