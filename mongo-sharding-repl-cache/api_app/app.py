...
async def startup():
    rd = aioredis.from_url(REDIS_URL, encoding="utf8", decode_responses=True)
    FastAPICache.init(RedisBackend(rd), prefix="api:cache")
...
async def list_users(collection_name: str):
    users_from_cache = await redis.get(collection_name)
    if users_from_cache:
        return UserCollection(users = json_util.loads(users_from_cache))
    time.sleep(1)
    collection = db.get_collection(collection_name)
    users_from_db = await collection.find().to_list(1000)
    await redis.set(collection_name, json_util.dumps(users_from_db))
    await redis.expire(collection_name, 60)
    return UserCollection(users = users_from_db)
